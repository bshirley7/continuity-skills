...
Only the human's explicit approval text and SSH signature from a project-trusted approver may populate a current approval record; an agent must never generate or infer them or select a signing key. In remote production mode, the local approver allowlist must match its fetched integration-branch copy byte-for-byte. `approval trust add` prepares a tracked change but grants no authority until protected human review merges it and the integration branch is fetched. Legacy unsigned approvals remain readable history but cannot authorize execution in a signed-approval project.
