...
| `$continuity-design` | An incomplete brief, captures, trusted memory, product intent, constraints, inspected current design, optional private research evidence, and installed catalog packs | Adaptive creative direction, private atlas/moodboard/concept/delta evidence, numbered feedback, passed native slop gates, explicit selection, and an exact document/reference/approval-bundle contract with retained rejected choices | `$continuity-plan` after exact bundle approval | Pause for visual feedback, ready-to-select direction choice, and exact bundle approval; third-party references remain private; contract-changing feedback invalidates prior evidence; revisions never alter goals automatically |
