...
| `$continuity-design` | An incomplete brief, captures, trusted memory, product intent, constraints, inspected current design, optional private research evidence, and installed catalog packs | Early creative-pathway diagnosis defaulting to new concepts from scratch, adaptive creative direction, evidence-checked brand guideline, private atlas/moodboard/concept/consultation/delta evidence, structured numbered feedback, passed native slop gates, explicit selection, and an exact document/reference/approval-bundle contract with retained rejected choices | `$continuity-plan` after exact bundle approval | Pause for consultation or conversational visual feedback, pending visual deltas, ready-to-select direction choice, and exact bundle approval; review or selected-concept refinement requires explicit user instruction and inherited-artifact provenance; preference is advisory; third-party references and consultation artifacts remain private; contract-changing feedback invalidates prior evidence; revisions never alter goals automatically |
