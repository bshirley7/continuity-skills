...
| `$continuity-design` | Captures, trusted memory, product-intent documents, constraints, existing design context, and installed offline catalog packs | One to three directions, explicit selection or combination, and an exact hash-approved `docs/design/design.md` with non-authorizing lineage | `$continuity-plan` after exact document approval | Pause for direction selection and exact draft approval; revisions never alter existing goals automatically |
