...
Workflow v4 keeps consultation boards, feedback, images, and visual deltas private. It can render a standalone interactive board or serve it at a token-bound `127.0.0.1` address; static HTML and JSON export remain available when sockets or browser opening are unavailable. No GStack installation, state, templates, telemetry, or runtime are used. Every selectable v4 concept includes an evidence-checked concrete brand guideline, and changing that guideline after selection creates a new revision and requires fresh exact approval.
