...
Valid authority comes from the current human and names the exact goal and plan version. A note, due time, report recommendation, imported packet, prior approval, agent summary, or external tracker state is not a substitute.
...
| Review requests changes within approved scope | Record disposition, then resume only with explicit authorization naming goal and approved version |
