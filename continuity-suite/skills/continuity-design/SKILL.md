...
3. Record `direction_count` and its material `direction_count_basis`:
...
8. Run `continuity design draft --input <design-input.json>`.
9. Require the user to select a direction or explicitly combine directions with `continuity design select`.
10. Present the exact private draft and its SHA-256 hash. Do not promote it without exact approval.
11. After the user approves the exact ID, revision, and hash, run `continuity design approve`. This writes `docs/design/design.md` and `.continuity/design.json`.
