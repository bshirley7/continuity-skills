...
5. **Directions and pre-consultation quality gate.** A fresh benchmark creates exactly three complete sequences; one-to-three is reserved for explicitly requested review or refinement. Every direction must demonstrate four or more slide roles and silhouettes, a peak, a quiet state, projected and read-ahead behavior, a sequence contact sheet, readable copy and data, and concept-forming media in at least one direction. Run the actual native slop command and obtain a screenshot-bound quality review from a human or independent agent other than the producer. Each direction must score at least 75, receive AI-slop grade A or B, clear every hard rejection, and state its user outcome, primary action, product proof, and five-second reaction.
