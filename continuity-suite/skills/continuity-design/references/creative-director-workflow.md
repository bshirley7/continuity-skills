...
- In a fresh formal benchmark, create exactly three equally developed boards and preselect no winner. Outside benchmarks, use two or three when material direction is unclear.
...
Before any concept reaches consultation, apply the GStack-derived embarrassment and hard-rejection gate to the rendered wide and narrow evidence. A reviewer other than the producer must verify the user outcome, primary action, realistic product proof, five-second reaction, expressive non-default type, body readability, genuine mobile transformation, brand strength, section variety, and absence of repeated mood claims. Formal benchmarks bind this as a quality review with score 75 or higher, every dimension at least 70, AI-slop grade A or B, a unique comparative rank, confidence, and a hash-bound raw-review attestation. Identical cross-concept warning waivers trigger a house-tell review. A declarative `passed` field, empty findings list, copied disposition, or agent self-review is not evidence.
...
For atlases, moodboards, concept comparisons, visual deltas, and every individual concept, capture valid browser-rendered PNG evidence at mobile, tablet, and desktop widths. Save the raw schema-4 JSON returned by `scripts/artifact-browser-probe.js` in each viewport and bind its file hash; free-form claims that the probe passed are invalid. The probe must report no clipping or content collisions, find the signature across opening, proof, interaction, quiet-or-edge, and closure, and measure the opening carrier inside the initial mobile and desktop viewport. Non-static interactions require separate before and after probe captures whose measurable state differs while protected invariants remain byte-equivalent. Review each concept's three captures together, then review the comparison set. Reusing the comparison probe as concept evidence is invalid. A source-valid HTML page without inspected browser output is not complete visual evidence.

## 6. Feedback and refinement

For new directions and material redesigns, render the native private consultation board from the current draft and validated evidence. Lead with large comparable opening visuals. Keep opening, responsive transformation, and an expandable complete journey or sequence together with concept-specific numbered feedback for the carrier, proof, transformation, and brand system; place rationale, safe/risk explanation, palette, typography, and brand rules behind progressive disclosure. The board supports refinement, more-like, remix, and start-over requests without running generation itself. Every selectable workflow-v4 concept carries a complete guideline whose palette, type transfer and license, assets, motion, primary carrier, responsive transformations, and provenance agree with the validated concept evidence.
...
Preserve accepted, rejected, and unresolved decisions in the active revision as well as its archive. Every `change`, `avoid`, refinement, more-like, remix, or start-over request is material unless refreshed evidence proves otherwise; it creates a new revision, clears prior selection, and invalidates affected concept and slop evidence. Redraft the same invalidated revision so feedback is carried forward rather than incrementing twice. Readiness requires refreshed evidence and every pending delta. Only feedback explicitly identified as human may mark readiness; agents may recommend but cannot self-certify their work. A preferred direction is advisory until the separate `design select` command.
