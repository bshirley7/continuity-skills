...
The installed design catalog selects one foundation pack per active axis plus reviewed category overlays matching the design input. Draft and approved records preserve every selected pack ID and version. Live research never runs inside the shared runtime: the agent gathers evidence, while Continuity renders private self-contained comparisons and validates local provenance and hashes. Human-selected concept-forming media becomes binding at equal or greater visual capability; Continuity checks its actual alpha or background integration and flags treatments that suppress defining material contrast. Film-led web work conditionally loads the cinematic-media method, and every modality passes the named AI-slop gate before concept selection, exact approval, and implementation-facing handoff.
...
Before merging a material Continuity-Design workflow change, run the applicable fixed benchmark: the versioned [Continuity homepage benchmark](skills/continuity-design/references/continuity-homepage-benchmark.md) for web/UI work, the [Continuity presentation benchmark](skills/continuity-design/references/continuity-presentation-benchmark.md) for deck/document/sequence work, and both for cross-modality changes. The presentation benchmark adds a narrative spine, four or more slide roles and silhouettes, concept-forming media, three preserved autonomous craft passes, separate projected and read-ahead review, encoded-format and frame-identity verification, and slide-by-slide copy and data readability proof before a contact sheet is accepted. When testing whether the workflow generalizes, use the improvement cycle's `fresh-design-experiments` mode so every round restarts from a new seed, reference set, moodboard, laboratory, and concept set. By round three, prove full design-system distance, deep interaction survival, and explicit review of recurring house tells. A valid intermediate stage is not a passing benchmark, and no benchmark stage authorizes implementation or publication.
