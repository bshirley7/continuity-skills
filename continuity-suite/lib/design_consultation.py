...
CSS = r"""
:root{color-scheme:light;--paper:#f2f1ec;--ink:#151917;--muted:#5d6762;--line:#bbc2bd;--signal:#1f5d4d;--danger:#a23b2b}*{box-sizing:border-box}body{margin:0;background:var(--paper);color:var(--ink);font-family:ui-sans-serif,system-ui,sans-serif}button,select,textarea{font:inherit}button,select,textarea{border:1px solid var(--ink);background:#fff;color:var(--ink)}button{padding:.75rem 1rem;cursor:pointer}button.primary{background:var(--ink);color:#fff}button:focus-visible,select:focus-visible,textarea:focus-visible,input:focus-visible,figure:focus-visible{outline:3px solid #ef9f35;outline-offset:3px}main{max-width:1600px;margin:auto;padding:clamp(1rem,4vw,4rem)}header{display:grid;grid-template-columns:minmax(0,1fr) minmax(20rem,.8fr);gap:2rem;border-bottom:2px solid var(--ink);padding-bottom:2rem}h1{font-size:clamp(2.5rem,7vw,6.5rem);line-height:.9;letter-spacing:-.05em;margin:.15em 0}.eyebrow{font:700 .72rem ui-monospace,monospace;letter-spacing:.15em;text-transform:uppercase}.lede{font-size:1.1rem;line-height:1.55;max-width:55ch}.notice{border-left:4px solid var(--signal);padding:1rem;background:#fff}.concepts{display:grid;gap:2rem;margin:2rem 0}.concept{background:#fff;border-top:5px solid var(--ink)}.concept-copy{padding:clamp(1rem,3vw,2rem)}.concept h2{font-size:clamp(2rem,4vw,4rem);margin:.3rem 0}.meta{display:flex;flex-wrap:wrap;gap:.5rem;margin:1rem 0}.tag{font:700 .7rem ui-monospace,monospace;border:1px solid var(--ink);padding:.35rem .5rem}.visuals{display:grid;grid-template-columns:2fr 1fr;background:#d8ddd9;gap:1px}.visuals figure{margin:0;background:#d8ddd9;height:clamp(24rem,68vh,48rem);overflow:auto;overscroll-behavior:contain;position:relative}.visuals img{display:block;width:100%;height:auto}.visuals figcaption{position:sticky;bottom:.5rem;left:.5rem;width:max-content;background:var(--ink);color:#fff;padding:.35rem .5rem;font-size:.72rem}.decision-grid,.guideline-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:1rem;margin-top:1rem}.panel{border:1px solid var(--line);padding:1rem}.panel h3{margin-top:0}.panel ul{padding-left:1.25rem}.risk{border-left:4px solid #ef9f35}.safe{border-left:4px solid var(--signal)}.swatches{display:flex;flex-wrap:wrap;gap:.5rem}.swatch{border:1px solid var(--line);padding:.6rem;min-width:9rem}.chip{display:block;width:100%;height:2.5rem;border:1px solid var(--line);margin-bottom:.4rem}.feedback{background:#101513;color:#f5f5ef;padding:clamp(1rem,4vw,3rem);margin-top:3rem}.feedback h2{font-size:clamp(2rem,5vw,4rem);margin-top:0}.reaction{display:grid;grid-template-columns:minmax(12rem,.6fr) minmax(10rem,.35fr) minmax(16rem,1fr);gap:1rem;align-items:start;border-top:1px solid #53615a;padding:1rem 0}.reaction label{display:grid;gap:.4rem}.reaction select,.reaction textarea{width:100%;padding:.65rem}.reaction textarea{min-height:5rem}.feedback-extra{display:grid;grid-template-columns:1fr 1fr;gap:1rem;margin-top:1rem}.feedback-extra label{display:grid;gap:.5rem}.feedback textarea,.feedback select{padding:.65rem}.actions{display:flex;flex-wrap:wrap;gap:.75rem;margin-top:1.25rem}.status{min-height:1.5rem;margin-top:1rem}.status.error{color:#ff9c83}.status.success{color:#8ee3b7}.quiet{color:var(--muted)}@media(max-width:850px){header,.visuals,.decision-grid,.guideline-grid,.feedback-extra,.reaction{grid-template-columns:1fr}.visuals figure{height:clamp(20rem,58vh,36rem)}}@media(prefers-reduced-motion:reduce){*{scroll-behavior:auto!important;transition:none!important;animation:none!important}}
"""


JS = r"""
(()=>{'use strict';const embedded=document.getElementById('continuity-board-data');const status=document.getElementById('status');let snapshot=null;const token=location.hash.startsWith('#token=')?decodeURIComponent(location.hash.slice(7)):'';if(token){history.replaceState(null,'',location.pathname)}const el=(name,attrs={},text='')=>{const node=document.createElement(name);for(const [key,value] of Object.entries(attrs)){if(key==='class')node.className=value;else if(key==='dataset')for(const [d,v] of Object.entries(value))node.dataset[d]=v;else node.setAttribute(key,String(value))}if(text)node.textContent=text;return node};const addList=(parent,items,formatter=x=>x)=>{const list=el('ul');for(const item of items||[])list.append(el('li',{},formatter(item)));parent.append(list)};const panel=(title,klass='panel')=>{const node=el('section',{class:klass});node.append(el('h3',{},title));return node};const renderConcept=(concept,index)=>{const article=el('article',{class:'concept'});const copy=el('div',{class:'concept-copy'});copy.append(el('span',{class:'eyebrow'},`${String(index+1).padStart(2,'0')} · ${concept.role.replace('-',' ')}`),el('h2',{},concept.title),el('p',{class:'lede'},concept.thesis));const tags=el('div',{class:'meta'});for(const value of [concept.recommended?'Recommended':null,concept.primary_carrier,concept.emotional_register].filter(Boolean))tags.append(el('span',{class:'tag'},value));copy.append(tags,el('p',{},concept.impact_thesis),el('p',{},`Tradeoff: ${concept.tradeoff}`));if(concept.consultation_summary){copy.append(el('h3',{},`Remember: ${concept.consultation_summary.memorable_thing}`),el('p',{},concept.consultation_summary.coherence_rationale));const decisions=el('div',{class:'decision-grid'});const safe=panel('Safe choices','panel safe');addList(safe,concept.consultation_summary.safe_choices,x=>`${x.decision} — ${x.rationale}`);const risks=panel('Creative risks','panel risk');addList(risks,concept.consultation_summary.creative_risks,x=>`${x.move} — gain: ${x.gain}; cost: ${x.cost}; boundary: ${x.boundary}`);decisions.append(safe,risks);copy.append(decisions)}if(concept.brand_guideline){const guide=el('div',{class:'guideline-grid'});const colors=panel('Color system');const swatches=el('div',{class:'swatches'});for(const color of concept.brand_guideline.color.roles){const swatch=el('div',{class:'swatch'});const chip=el('span',{class:'chip'});chip.style.background=color.value;swatch.append(chip,el('strong',{},color.name),el('div',{class:'quiet'},`${color.token} · ${color.value}`),el('p',{},color.usage));swatches.append(swatch)}colors.append(swatches);const type=panel('Typography');addList(type,concept.brand_guideline.typography.roles,x=>`${x.role}: ${x.family} ${x.weight_style} — ${x.usage}`);const spatial=panel('Spacing and layout');spatial.append(el('p',{},`Base ${concept.brand_guideline.spatial.base_unit}. ${concept.brand_guideline.spatial.grid}`));addList(spatial,concept.brand_guideline.spatial.transformations,x=>`${x.context}: ${x.rule}`);const behavior=panel('Motion and voice');behavior.append(el('p',{},`${concept.brand_guideline.motion.status}: ${concept.brand_guideline.motion.rationale}`),el('p',{},`Reduced motion: ${concept.brand_guideline.motion.reduced_motion}`));addList(behavior,concept.brand_guideline.voice.principles);guide.append(colors,type,spatial,behavior);copy.append(guide)}article.append(copy);const visuals=el('div',{class:'visuals'});for(const image of concept.images){const figure=el('figure',{tabindex:'0','aria-label':`${image.label}. Scroll to inspect the full evidence.`});figure.append(el('img',{src:image.src,alt:image.alt}),el('figcaption',{},image.label));visuals.append(figure)}article.append(visuals);return article};const reactionRow=item=>{const row=el('div',{class:'reaction',dataset:{elementId:item.element_id}});row.append(el('div',{},item.label));const choiceLabel=el('label',{},'Reaction');const select=el('select',{'aria-label':`Reaction for ${item.label}`});select.append(el('option',{value:''},'Choose…'));for(const value of ['keep','change','avoid','uncertain'])select.append(el('option',{value},value));choiceLabel.append(select);const whyLabel=el('label',{},'Why');whyLabel.append(el('textarea',{'aria-label':`Reason for ${item.label}`,placeholder:'Use your own words. What should stay or change?'}));row.append(choiceLabel,whyLabel);return row};const render=data=>{snapshot=data;document.getElementById('title').textContent=data.title;document.getElementById('context').textContent=data.intent;document.getElementById('revision').textContent=`Design ${data.design_id} · revision ${data.revision}`;const concepts=document.getElementById('concepts');for(const [index,concept] of data.concepts.entries())concepts.append(renderConcept(concept,index));const form=document.getElementById('feedback-form');for(const item of data.elements)form.append(reactionRow(item));const preferred=document.getElementById('preferred');preferred.append(el('option',{value:''},'No preference yet'));for(const concept of data.concepts.filter(x=>x.direction_id))preferred.append(el('option',{value:concept.direction_id},concept.title));document.getElementById('submit').disabled=!token};const payload=()=>{const reactions=[];for(const row of document.querySelectorAll('.reaction')){const reaction=row.querySelector('select').value;const why=row.querySelector('textarea').value.trim();if(reaction){if(!why)throw new Error(`Add a reason for ${row.firstChild.textContent}`);reactions.push({element_id:row.dataset.elementId,reaction,why,user_language:why})}}if(!reactions.length)throw new Error('Choose at least one numbered reaction.');const changed=reactions.some(x=>x.reaction==='change'||x.reaction==='avoid');return{design_id:snapshot.design_id,revision:snapshot.revision,board_hash:snapshot.board_hash,concept_evidence_hash:snapshot.concept_evidence_hash,actor:snapshot.actor,reactions,contract_changed:changed,ready_for_selection:changed?false:document.getElementById('ready').checked,preferred_direction_id:document.getElementById('preferred').value,overall_notes:document.getElementById('overall').value.trim()}};const download=value=>{const blob=new Blob([JSON.stringify(value,null,2)+'\n'],{type:'application/json'});const link=el('a',{href:URL.createObjectURL(blob),download:`${snapshot.design_id}-feedback-r${snapshot.revision}.json`});document.body.append(link);link.click();link.remove();setTimeout(()=>URL.revokeObjectURL(link.href),0)};document.getElementById('export').addEventListener('click',()=>{try{download(payload());status.textContent='Feedback JSON exported. Record it with continuity design feedback-record.';status.className='status success'}catch(error){status.textContent=error.message;status.className='status error'}});document.getElementById('submit').addEventListener('click',async()=>{try{const value=payload();const response=await fetch('/api/v1/feedback',{method:'POST',headers:{Authorization:`Bearer ${token}`,'Content-Type':'application/json'},body:JSON.stringify(value)});const result=await response.json();if(!response.ok)throw new Error(result.error||'Feedback submission failed.');status.textContent=`Feedback round ${result.feedback_round} captured. This is not design approval. Status: ${result.status}.`;status.className='status success';document.getElementById('submit').disabled=true}catch(error){status.textContent=error.message;status.className='status error'}});const load=async()=>{try{const text=embedded.textContent.trim();if(text){render(JSON.parse(text));return}if(!token)throw new Error('Consultation authorization is missing. Reopen the URL printed by Continuity.');const response=await fetch('/api/v1/board',{headers:{Authorization:`Bearer ${token}`}});const data=await response.json();if(!response.ok)throw new Error(data.error||'Could not load consultation board.');for(const concept of data.concepts){for(const image of concept.images){const media=await fetch(`/api/v1/${image.src}`,{headers:{Authorization:`Bearer ${token}`}});if(!media.ok)throw new Error('Could not load validated consultation media.');image.src=URL.createObjectURL(await media.blob())}}render(data)}catch(error){status.textContent=error.message;status.className='status error'}};load()})();
"""
...
def _document(embedded: dict[str, Any] | None) -> str:
    ...
    data = data.replace("<", "\\u003c")
    return f'''<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Continuity Design consultation</title><style>{CSS}</style></head><body><main><header><div><span class="eyebrow" id="revision">Private design consultation</span><h1 id="title">Continuity Design</h1></div><div><p class="lede" id="context">Loading current design evidence…</p><p class="notice">React in your own words. Feedback may create a new private revision, but this board cannot select, approve, publish, or authorize implementation.</p></div></header><section id="concepts" class="concepts" aria-label="Design concepts"></section><section class="feedback" aria-labelledby="feedback-title"><h2 id="feedback-title">Numbered feedback</h2><p>Choose only the elements you want to address. A reason is required for every reaction.</p><div id="feedback-form"></div><div class="feedback-extra"><label>Preferred direction (advisory only)<select id="preferred"></select></label><label>Overall notes<textarea id="overall" rows="5" placeholder="What should the next revision feel or do differently?"></textarea></label></div><label><input id="ready" type="checkbox"> The refreshed concept set is ready for the separate selection step.</label><div class="actions"><button id="submit" class="primary" type="button">Submit privately</button><button id="export" type="button">Export feedback JSON</button></div><p id="status" class="status" role="status" aria-live="polite"></p></section></main><script id="continuity-board-data" type="application/json">{data}</script><script>{JS}</script></body></html>'''
