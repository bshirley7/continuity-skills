...
def command_index(root: Path, config: dict[str, Any], _args: argparse.Namespace) -> dict[str, Any]:
    value = projection(root, config)
    return {"entries": len(value["entities"]), "goals": len(value["goals"]), "note_links": len(value["note_links"]), "shared_packets": len(value["shared_packets"]), "projection": runtime_lib.project_relative_posix(_private(root, config) / "roadmap" / "projection.json", root)}
