...
7. Demonstrate conformance by allowing `project doctor` to observe at least two successive sweeps, launching a claimed no-op review or report action, rejecting a replayed claim, showing that an expired supervisor becomes `stale`, and proving a second workstation cannot acquire an active execution lease. Record the three rejection observations with `scheduler adapter codex record-probe`; each production record must carry concrete evidence, an actor, the current behavior hash, an artifact SHA-256, and a trusted SSH signature.
