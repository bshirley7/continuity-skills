...
For Codex, generate the task definition with `scheduler adapter codex render` and verify observed operation with `scheduler adapter codex verify`. Verification is unhealthy until the sweeps, claimed no-op, claim-replay rejection, stale-registration rejection, and remote-lease-contention rejection are all evidenced for the current behavior configuration. Do not hand-maintain a divergent prompt or schedule when a rendered definition is available.
