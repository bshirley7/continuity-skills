...
1. Triage new captures conservatively and use derived `note queue` results rather than queue JSONL history; notes never authorize work.
...
11. Finish with `workflow status` and report its next skill, blockers, and human-required actions without executing them.
