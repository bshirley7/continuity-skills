...
Material Continuity-Design workflow changes also run the versioned [Continuity homepage benchmark](skills/continuity-design/references/continuity-homepage-benchmark.md) before merge. Version 8 binds the source skill, fixed brief, seed, project-doctor result, style-frame breadth, rendered type, exact single-source reconstruction, two visual-difference corrections, measured transfer grammar, literal project substitution, controlled divergence, independent combined-pixel review, page-grammar and deep-journey structure, interaction range, generated-media disposition, full-page comparison depth, multimodal impact, concept-specific viewport probes, private prototype, six evidence roles, exact human approval, hard-failure dispositions, and outside human score. Intermediate gates may validate, but only a score of 75 or greater with no hard failure is merge-eligible. The run stays private and never authorizes homepage implementation.
