...
$continuity-design    direct, visualize, portfolio-check, refine, slop-check, and exactly approve design (optional collection)
```
...
Projects that need design direction can enable the optional collection with `--collection design`. `$continuity-design` operates as an adaptive creative director: one sentence is enough, new concepts from scratch are the default, and review or selected-concept refinement requires an explicit user instruction, live reference research is announced and opt-out, named references are decomposed rather than imitated, and private atlases, moodboards, concept comparisons, and visual deltas let users react without design taxonomy. Before directions, a hash-bound generative concept laboratory uses image generation, image editing, SVG, rendered type specimens, motion prototypes, and code studies to widen the search space. Its range plan requires materially different typography, art direction, composition, target grammar, interaction, and journey hypotheses before concepts converge. Promising generated families require multiple style frames, selected and rejected alternatives, explicit `concept-forming`, `supporting`, or `rejected` disposition, and system extraction rather than silent pixel promotion. Workflow v3 then privately reconstructs the defining reference mechanics in responsive HTML/CSS, classifies medium-specific difficulty, compiles measured constraints, and requires quantified close and far adaptations before full concept expansion. A cross-run portfolio gate fingerprints prior outputs, detects Continuity house tells, and requires the strongest current concept to challenge the strongest prior generation. This prevents image-led or materially rich studies from collapsing into primitive CSS decoration, timid cosmetic variance, or a recurring house template. Once a human selects concept-forming media for its impact, the media or a provenance-preserving edit at equal or greater capability becomes binding; the workflow verifies real alpha or background behavior and rejects integration treatments that erase defining material contrast. A separate creative-range and multimodal impact gate proves distinct systems, concept-forming media, full journeys, purposeful-motion or explicit no-motion evidence, signature survival across downstream roles and states, per-concept responsive evidence, and at least one compelling direction. Presentation work is first-class: HTML/CSS remains a private carrier, while the workflow requires a narrative spine, at least four slide roles and silhouettes, one peak, one quiet or rest state, projected and read-ahead reviews, individually verified captures, and a contact sheet assembled from those frames. Ambiguous work receives two or three equally developed concepts; clear work receives a recommendation plus deliberate contrasts. Numbered feedback creates durable revisions, and a native network-free AI-slop gate checks concept, prototype, and implementation evidence for hard failures, unexplained defaults, translation collapse, presentation-template reflexes, invalid evidence, and project drift. A native `design improvement-cycle` record makes repeated diagnose, change, benchmark, critique, and human-review passes resumable while preserving exact evidence and authority gates. It defaults to `fresh-design-experiments`, which requires every round to use a new creative seed, reference set, moodboard, laboratory, concept set, and comparison so cross-cycle learning cannot be simulated by polishing one artifact. By the third round it also requires full design-system distance, interaction survival across the journey, and an explicit house-tell review; `artifact-refinement` is opt-in and requires the user's explicit instruction for one selected design. The committed schema-v2 `.continuity/design.json` binds the document, eligible owned visual references, ruleset, and combined approval bundle. Design approval authorizes only that exact contract and reference bundle; implementation still requires a separately approved goal bound to the approval-bundle hash.

Workflow v4 adds a native consultation layer without adding a service dependency. Every selectable concept carries one memorable outcome, a coherent recommendation, explained safe choices, bounded creative risks, and a concrete brand guideline cross-checked against validated color, type, assets, motion, carrier, responsive behavior, and provenance. `design consultation-render` creates a private interactive HTML board with JSON export; `design consultation-serve` optionally accepts the identical normalized feedback over a hardened token-bound loopback endpoint. Material feedback is recorded before refinement and may leave a visual delta pending until `design feedback-delta-record` binds the after evidence. Advisory preference never selects, and the board never approves, publishes, generates, or authorizes implementation. Versions 1-3 remain readable and the runtime has no GStack dependency.

`scripts/evaluate_design_scenarios.py` measures deterministic workflow, routing, and regression behavior; it does not claim that the rendered work is aesthetically better. Continuity Design and its versioned benchmarks default to new concepts from scratch. An early consultation diagnosis can route to existing-design review or selected-concept refinement only when the user explicitly asks; those paths bind inherited-artifact provenance and cannot silently masquerade as fresh benchmark output. Material output-improvement claims require a separate blinded paired benchmark over real hash-bound baseline and candidate renders using `scripts/evaluate_design_outputs.py --root <benchmark-root> --manifest <benchmark.json>`. The benchmark fails when the candidate lacks sufficient reviewer lift, win rate, or regresses a quality dimension.

Presentation and pitch-deck validation additionally fails closed when any audience-facing title, body line, label, value, source, or qualification is hidden, clipped, out of frame, undersized, low-contrast, or rendered with a font fallback. The browser probe records slide-by-slide copy and data readability at projection, tablet read-ahead, and mobile read-ahead widths; a contact sheet cannot substitute for those individual checks.

Material Continuity-Design workflow changes run the applicable fixed benchmark before merge: the versioned [Continuity homepage benchmark](skills/continuity-design/references/continuity-homepage-benchmark.md) for web/UI work, the [Continuity presentation benchmark](skills/continuity-design/references/continuity-presentation-benchmark.md) for document/sequence/deck work, and both for cross-modality changes. The presentation benchmark binds source and brief hashes, reference reconstruction, concept-forming media, slide-role and silhouette range, three autonomous craft passes, projected and read-ahead evidence, individual-frame capture integrity, native slop evidence, exact approval, hard-failure dispositions, and outside human score. Intermediate gates may validate, but only a score of 75 or greater with no hard failure is merge-eligible. Runs stay private and never authorize implementation or publication.
